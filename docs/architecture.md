...
`/config.json` is fetched with `cache: no-store` before rendering. It is public configuration, not a secret mechanism. The production container permits this file to be replaced with a ConfigMap or bind mount while hashed application assets remain immutable. Emoji pack enablement and asset hosting are runtime-configurable; disabled packs are never fetched.
