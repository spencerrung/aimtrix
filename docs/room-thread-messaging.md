...
The disposable Synapse/Dex harness is the live protocol gate. Its browser devices and direct Matrix API peer are distinct from the optional Element Web UI check. That check sends formatted messages through both applications' actual composers and verifies their rendered structure in the receiving client. See [live Matrix tests](live-matrix-tests.md) for the pinned Element version and isolated environment.

[Delivery MR #193](https://github.com/spencerrung/aimtrix/pull/193) records the live run and hosted CI results. Local validation passed `npm run check` with 888 unit tests and the production build/bundle budgets, and `npm run test:e2e` with 107 passes and 7 intentional platform-specific skips. Desktop/mobile and short-height screenshots were inspected. Live validation also exposed and corrected fresh-thread link classification before SDK thread metadata/remote echo arrives; known roots remain scoped to their room.

Native hardware, platform clipboard providers, federation and other third-party clients retain their separate acceptance checks. Test reports state which gates actually ran; fixture success alone does not prove those boundaries. No production deployment or live account migration is part of this delivery.
