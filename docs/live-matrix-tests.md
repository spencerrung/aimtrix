...
These journeys are test coverage pending execution at the final built revision; this section does not pre-claim a live pass. Federated alias routing, unavailable rooms, native OS link dispatch, and physical-device keyboard behavior retain their separate evidence boundaries.
